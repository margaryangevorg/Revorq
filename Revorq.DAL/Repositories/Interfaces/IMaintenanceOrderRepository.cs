...
public interface IMaintenanceOrderRepository : IRepository<MaintenanceOrder>
{
    ...
    Task<IEnumerable<MaintenanceOrder>> GetMonthlyOrdersAsync(int userId, int? engineerId, int year, int month, OrderStatus? status, bool? isUnassigned, bool? isScheduled);
